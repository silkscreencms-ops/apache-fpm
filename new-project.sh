...
<VirtualHost *:80>
        ...
        ServerName @@PROJECT@@.vm

        ServerAdmin @@USER@@@localhost
        DocumentRoot @@HOME@@/@@PROJECT@@/docroot/
        ...
        <Directory @@HOME@@/@@PROJECT@@/docroot>
        ...
        <FilesMatch ".+\.ph(p[3457]?|t|tml)$">
                SetHandler "proxy:unix:/run/php/@@USER@@-fpm.sock|fcgi://localhost"
        </FilesMatch>
...
create_pool() {
	echo Creating FPM Pool $1
	cat > ${FPM_ROOT}/pool.d/$1 << EOF
; Start a new pool named '@@USER@@'.
